...
/// Swift Concurrency-safe page store with async file operations
struct PageStore {
    private init() { }

    static func save(tabs: [Tab]) async {
        await Task.detached {
            createDirectory()
    ...
    static func save(currentTab: Int?) async {
        await Task.detached {
    ...
    static func save(favourites: [Page]) async {
        await Task.detached {
            createDirectory()
    ...
    static func save(history: [Date: Page]) async {
        await Task.detached {
            createDirectory()
