...
/// Swift Concurrency-safe with async/await for timing operations
@available(iOS 16.0, *)
public struct EcosiaErrorToast: View {
    ...
    public var body: some View {
        ...
        .onAppear {
            ...
            Task {
                try? await Task.sleep(for: .seconds(UX.displayDuration))
                dismiss()
            }
        }
    }
    ...
    private func dismiss() {
        ...
        Task {
            try? await Task.sleep(for: .seconds(UX.animationDuration))
            onDismiss()
        }
    }
}
