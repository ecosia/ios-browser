...
CURRENT_VERSION=$(grep 'MARKETING_VERSION' firefox-ios/Client/Ecosia/BuildSettingsConfigurations/EcosiaCommon.xcconfig | cut -d ' ' -f3)
...
if [ -n "$CIRCLECI" ]; then
  ...
  if git rev-parse "$CIRCLE_BRANCH~1" >/dev/null 2>&1; then
    OLD_VERSION=$(git show "$CIRCLE_BRANCH~1:firefox-ios/Client/Ecosia/BuildSettingsConfigurations/EcosiaCommon.xcconfig" | grep 'MARKETING_VERSION' | cut -d ' ' -f3)
  else
