...
We use [Tuist](https://tuist.dev) to generate the Xcode project and manage the build setup. Tuist is an iOS tooling platform that generates workspaces and projects from Swift-based manifests, keeping the project structure consistent and making dependency management easier. The `Client.xcodeproj` is generated by Tuist—do not edit it by hand; use the Tuist manifests (e.g. [Project.swift](../../Project.swift)`) and run the setup script when you need to regenerate. [See a map here](./project-structure.mmd)
