...
/// Constants used for authentication state notifications and communication
public enum EcosiaAuthConstants {
    ...
    public enum State: String {
        case authenticationStarted = "authenticationStarted"
...
extension AuthUser: CustomStringConvertible {
    public var description: String {
        let redactedIdToken = idToken?.prefix(10).appending("***[REDACTED]") ?? "nil"
        let redactedAccessToken = accessToken?.prefix(10).appending("***[REDACTED]") ?? "nil"
        return "AuthUser(idToken: \(redactedIdToken), accessToken: \(redactedAccessToken))"
    }
}
...
public final class AuthStateManager {
    ...
    /// Directly updates the authentication state (for advanced use cases)
    /// - Parameter state: New authentication state
    public func updateAuthState(_ state: AuthState) {
        updateState(state)
    }
