...
// Ecosia: Vendored from Firefox's `Client/Frontend/Browser/SimpleToast.swift`, which upstream
// deleted in 155.1 when it moved its own callers onto the delegate-based `showToast(message:)`
// plumbing around `Toast`. Ecosia still uses this simpler API in four places (bookmarks import/
// export, settings, and `SimpleToast+Ecosia`), so the type is kept here unchanged rather than
// migrating those call sites during an upgrade. It depends only on `Toast.UX`, which still exists.
