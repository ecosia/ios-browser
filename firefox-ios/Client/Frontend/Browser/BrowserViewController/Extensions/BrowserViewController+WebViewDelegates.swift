...
// MARK: - Ecosia Auth URL Detection
extension BrowserViewController {
    
    /// Detects Ecosia authentication URLs and triggers appropriate native flows
    /// This is simpler than the WebAuthURLDetector as it's called directly from navigation delegate
    private func detectAndHandleAuthURL(_ url: URL) {
        guard url.isEcosia() else { return }
        
        let path = url.path.lowercased()
        
        // Sign-in URL patterns
        let signInPaths = ["/accounts/sign-up", "/login", "/signin", "/auth/login"]
        let signOutPaths = ["/accounts/sign-out", "/logout", "/signout", "/auth/logout"]
        
        if signInPaths.contains(where: { path.contains($0) }) {
            handleSignInDetection(url)
        } else if signOutPaths.contains(where: { path.contains($0) }) {
