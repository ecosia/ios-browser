...
final class AuthTests: XCTestCase {
    ...
    func testRenewCredentialsIfNeeded_whenConversationScopesMissing_requestsUpdatedScopesSilently() async throws {
        let oldToken = try makeJWT(claims: ["scope": "openid profile email"])
        let newToken = try makeJWT(claims: ["scope": EcosiaAuthScopes.oauthScope])
        ...
        XCTAssertEqual(mockProvider.lastRenewScope, EcosiaAuthScopes.oauthScope)
        XCTAssertEqual(auth.accessToken, newToken)
        XCTAssertTrue(EcosiaAuthScopes.hasConversationScopes(in: auth.accessToken ?? "", grantedScope: auth.grantedScope))
    }

    func testRenewCredentialsIfNeeded_whenConversationScopesOnlyInGrantedScope_acceptsRenewal() async throws {
        let oldToken = try makeJWT(claims: ["scope": "openid profile email"])
        let opaqueToken = "opaque-api-access-token"
        mockProvider.mockCredentials = Credentials(
            accessToken: oldToken,
            tokenType: "Bearer",
            idToken: "test-id-token",
        ...
        XCTAssertEqual(auth.accessToken, opaqueToken)
        XCTAssertTrue(EcosiaAuthScopes.hasConversationScopes(in: opaqueToken, grantedScope: auth.grantedScope))
    }

    func testRenewCredentialsIfNeeded_whenAudienceConfigured_usesAPICredentials() async throws {
        mockProvider.authApiAudience = "https://auth0.api.ecosia.org/v1/accounts/web"
        let oldToken = try makeJWT(claims: ["scope": "openid profile email"])
        let newToken = "audience-scoped-token"
        mockProvider.mockCredentials = Credentials(
            accessToken: oldToken,
            tokenType: "Bearer",
            idToken: "test-id-token",
        ...
        XCTAssertEqual(mockProvider.renewCredentialsWithScopeCallCount, 1)
        let mockCredentialsManager = mockProvider.credentialsManager as? MockCredentialsManager
        XCTAssertEqual(mockCredentialsManager?.apiCredentialsCallCount, 1)
        XCTAssertEqual(mockCredentialsManager?.lastRequestedAudience, mockProvider.authApiAudience)
        XCTAssertEqual(auth.accessToken, newToken)
    }
