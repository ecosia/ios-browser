...
class TabManager: NSObject, FeatureFlagsProtocol {
    ...
    func removeTabsWithToast(_ tabs: [Tab]) {
        ...
        if numberOfTabs > 0 {
            toast = ButtonToast(labelText: String.localizedStringWithFormat(Strings.TabsDeleteAllUndoTitle, numberOfTabs), imageName: "check", buttonText: Strings.TabsDeleteAllUndoAction, completion: { buttonPressed in
                if buttonPressed {
                    self.undoCloseTabs()
                    self.storeChanges()
...
extension TabManager {
    ...
    func restoreTabs(_ forced: Bool = false) {
        defer {
            ...
            if normalTabs.isEmpty || !User.shared.seenRebrandIntro {
                let tab = addTab()
                if selectedTab == nil || !User.shared.seenRebrandIntro {
                    selectTab(tab)
                }
            }
        }
