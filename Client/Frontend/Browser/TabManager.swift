...
class TabManager: NSObject, FeatureFlagsProtocol {
    ...
    func removeTabsWithToast(_ tabs: [Tab]) {
        ...
        if numberOfTabs > 0 {
            toast = ButtonToast(labelText: String.localizedStringWithFormat(Strings.TabsDeleteAllUndoTitle, numberOfTabs), imageName: "tabs", buttonText: Strings.TabsDeleteAllUndoAction, completion: { buttonPressed in
                if buttonPressed {
                    self.undoCloseTabs()
                    self.storeChanges()
